    ...
    if(bgTypeId >= MAX_BATTLEGROUND_TYPES)
    {
        ...
        if(_player->InBattleGroundQueue())
        {
            ...
            for (uint32 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; i++)
            {
                ...
                if(!queue_id)
                    continue;
                BattleGroundQueue::QueuedPlayersMap::iterator itrPlayerStatus = sBattleGroundMgr.m_BattleGroundQueues[queue_id].m_QueuedPlayers[_player->GetBattleGroundQueueIdFromLevel(queue_id)].find(_player->GetGUID());
                // if the player is not in queue, contine
                if(itrPlayerStatus == sBattleGroundMgr.m_BattleGroundQueues[queue_id].m_QueuedPlayers[_player->GetBattleGroundQueueIdFromLevel(queue_id)].end())
                    continue;
    ...
    if(_player->InBattleGroundQueue())
    {
        ...
        switch(action)
        {
            case 1:                                     // port to battleground
                ...
                sBattleGroundMgr.m_BattleGroundQueues[bgQueueTypeId].RemovePlayer(_player->GetGUID(), false, bgQueueTypeId);
            ...
            case 0:                                     // leave queue
                ...
                sBattleGroundMgr.m_BattleGroundQueues[bgQueueTypeId].RemovePlayer(_player->GetGUID(), true, bgQueueTypeId);
